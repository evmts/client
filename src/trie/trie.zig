...
pub const BranchNode = struct {
    ...
    pub fn encode(self: BranchNode, allocator: Allocator) ![]u8 {
        ...
        for (self.children) |child| {
            if (child) |value| {
                switch (value) {
                    ...
                    .Hash => |hash| {
                        const encoded = try rlp.encodeBytes(allocator, &hash);
                        try encoded_children.append(encoded);
                    },
                }
            } else {
                const empty = try rlp.encodeBytes(allocator, "");
        ...
        return try rlp.encodeList(allocator, encoded_children.items);
    }
};
...
pub const ExtensionNode = struct {
    ...
    pub fn encode(self: ExtensionNode, allocator: Allocator) ![]u8 {
        ...
        const encoded_next = switch (self.next) {
            .Raw => |data| try rlp.encodeBytes(allocator, data),
            .Hash => |hash| try rlp.encodeBytes(allocator, &hash),
        };
        try items.append(encoded_next);

        // Encode as a list
        return try rlp.encodeList(allocator, items.items);
    }
};
...
pub const TrieNode = union(NodeType) {
    ...
    pub fn encode(self: TrieNode, allocator: Allocator) ![]u8 {
        return switch (self) {
            .Empty => try rlp.encodeBytes(allocator, ""),
